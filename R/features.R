...
plot_module_preservation <- function(module_preservation_results,
    ...
    medianrank_plot <- .make_medianrank_plot(mr_df, max_module_size, nudge_mr)
    zsummary_plot <- .make_zsummary_plot(zs_df, max_module_size, nudge_zs)
...
#' @noRd
#' @keywords internal
.make_zsummary_plot <- function(zs_df, max_module_size, nudge_zs) {
    ggplot(zs_df, aes(
        !!sym("moduleSize"),
        !!sym("Zsummary.pres"),
        col = !!sym("module")
