...
`estimate_factors`, for identifying factors through Independent Component
Analysis. We also provide a wrapper for applying WGCNA, `identify_modules`.
...
```{r estimate_stability, message=FALSE, warning=FALSE}
...
These plots were made for a small test dataset. For standard datasets with
thousands of genes, you might see a stability profile more like the one
plotted below. You'll commonly observe an elbow in the stability plot on the
left, which in this case appears around 30-40 components.

![](https://raw.githubusercontent.com/jackgisby/ReducedExperiment/devel/inst/stability.png)
...
First, we use the `assess_soft_threshold` function to determine the 
...
WGCNA's `pickSoftThreshold` function for more details.

```{r assess_threshold}
WGCNA::disableWGCNAThreads()

wave1_fit_indices <- assess_soft_threshold(wave1_se)
wave1_estimated_power <- wave1_fit_indices$Power[wave1_fit_indices$estimated_power]

print(paste0("Estimated power: ", wave1_estimated_power))
```

Now we can pass our data with the selected power to the `identify_modules`
function. Since for this example we've filtered out most of the genes that were in
the original dataset, we allow for the identification of smaller modules of 
genes (10). The dendrogram is also stored in this object, allowing us to plot the clusters
...
and we've used a different soft thresholding power. While `identify_modules`
...
`associate_components` function. First, we can look at the sample-level data 
available in the `SummarizedExperiment` container.

```{r show_coldata}
colData(wave1_se)[, c("sample_id", "individual_id", "case_control", "sex", "calc_age")]
```
...
associations_fe <- associate_components(wave1_fe, method = "lm", formula = f)
```

Below is a table for the significant associations between the factors and the `case_control` 
...
# We run GSEA for module 5 only as an example
factor_enrich <- runEnrich(wave1_fe[, , 5], as_dataframe = TRUE)

# We run overrepresentation analysis for all the modules
...
ggplot(
    module_enrich[module_enrich$component == "module_1" & module_enrich$p.adjust < 0.05, ][1:15, ],
...
The new `FactorisedExperiment` created by projecting the wave 2 data (above) has the
same genes (2184) and factors (34) as the original `FactorisedExperiment` (below),
but with different samples (83 vs. 65). 
...
replication_fe <- associate_components(wave2_fe, method = "lmer", formula = paste0(f, " + (1|individual_id)"))
```
...
```{r calc_eigengenes_new_data}
wave2_me <- calcEigengenes(wave1_me, wave2_se)
replication_me <- associate_components(wave2_me, method = "lmer", formula = paste0(f, " + (1|individual_id)"))
...
`modulePreservation`. By default, the `plot_module_preservation` function 
...
# Using a low number of permutations (20) for the vignette
mod_pres_res <- module_preservation(wave1_me, wave2_se, verbose = 0, nPermutations = 20)
...
```{r extension_vignette, eval=FALSE}
vignette("Extensions", package = "SummarizedExperiment")
```

The `ReducedExperiment` containers may be extended in a similar fashion. In this
package, we implemented `ReducedExperiment` and two derivative classes, 
...
is now ready to work with as described in the section above ("Introducing `ReducedExperiment` containers with a case study").
