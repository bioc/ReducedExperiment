...
Or the development version may be installed, from 
...
```{r estimate_stability, message=FALSE, warning=FALSE, include=FALSE}
...
Note that, for the sake of displaying a stability profile representative of 
real data, the plot above was generated for the full dataset, rather than the 
smaller test dataset we are using in this vignette. 
You'll commonly observe an elbow in the stability plot on the
...
mod_1_enrich <- module_enrich[
    module_enrich$component == "module_1" & 
    module_enrich$p.adjust < 0.05, 
]
...
has the same genes (2184) and factors (34) as the original 
`FactorisedExperiment` (below),
